...
(async()=>{
 ...
 assert.equal(claimed,false,'verification must not bind before Passkey login and confirmation');
 assert.equal(await page.locator('#existingVpnPassword').inputValue(),'');
 ...
 assert.deepEqual(errors,[]);
 await browser.close();
 console.log('PASS: password verification, admin-only protected users, invitation survives login, explicit claim, three languages, protected controls, narrow layout');
