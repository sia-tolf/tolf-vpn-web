...
(async()=>{
 ...
 assert.equal(await page.locator('[data-node=moscow] tbody tr').count(),2);
 ...
 assert.equal(await page.getByRole('button',{name:'Отключить Test',exact:true}).count(),1);
 page.once('dialog',dialog=>dialog.dismiss());
 await page.getByRole('button',{name:'Отключить Test',exact:true}).click();
 await page.waitForFunction(()=>!Array.from(document.querySelectorAll('button')).find(b=>b.textContent==='Отключить Test').disabled);
 assert.equal(disconnectPosts,0);
 await page.screenshot({path:'/tmp/tolf-admin-test-control.png',fullPage:true});
