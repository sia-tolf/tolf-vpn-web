...
int wmain(){std::wstring name,moscowName;bool owned=false,moscowOwned=false;try{
 ...
 mc.displayName=L"Тестовый компьютер";mc.routingMode=L"ru";SaveProfileLabels(mc);
 Check(ReadLabelValue(mc.id,L"DisplayName")==mc.displayName);Check(ReadLabelValue(mc.id,L"RoutingMode")==L"ru");
 Check(ProfileName(mc)==moscowName);Check(SavedNetworks(mc.id)==NetworkText(passwordRoutes));
