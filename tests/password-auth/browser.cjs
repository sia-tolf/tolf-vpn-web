...
(async()=>{
 ...
 try {
  for(const lang of ['ru','en','lv']) {
   ...
   assert.equal(await page.locator('#copyCredentialsButton,#copyRecoveryButton').count(),0);
   assert.equal(await page.locator('#recoveryCodePanel button').count(),2);
   assert(content.includes('https://vpn.tolf.is/auth/'));
   assert.equal(download.suggestedFilename(),'TOLF-account-recovery.txt');
