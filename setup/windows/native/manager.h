...
namespace Manager {
static HWND wnd,select,stateText,details,netLabel,netEdit,help,save,toggle,mode,autostart,pin,message,technical,divider,location,passwordButton,passwordLabel,passwordEdit,passwordSave,passwordCancel;
static HFONT normal,heading;static HBRUSH background;
static std::vector<LocalProfile> profiles;
static bool compact=false,working=false,dirty=false,trayPresent=false,showDetails=false,passwordOpen=false;
...
static std::wstring ProfileLabel(const LocalProfile& p){
 if(p.displayName.empty())return std::wstring(NodeLabel(p.server))+L" · "+p.id.substr(0,8);
 auto routing=p.routingMode.empty()?std::wstring(L"AUTO"):p.routingMode;
 for(auto& ch:routing)if(ch>=L'a'&&ch<=L'z')ch-=L'a'-L'A';
 return p.displayName+L" · "+NodeLabel(p.server)+L" · "+routing;
}
...
static void ClearPassword() {passwordOpen=false;if(passwordEdit)SetWindowTextW(passwordEdit,L"");}
static void Layout() {
    for(HWND c:{netLabel,netEdit,help,autostart,technical,divider,passwordButton})ShowWindow(c,compact?SW_HIDE:SW_SHOW);
    for(HWND c:{passwordLabel,passwordEdit,passwordSave,passwordCancel})ShowWindow(c,!compact&&passwordOpen?SW_SHOW:SW_HIDE);
    ...
    int width=compact?420:560;
    int footer=dirty?382:334;
    int height=compact?244:footer+130+(showDetails?112:0)+(passwordOpen?174:0);
    MoveWindow(select,Px(24),Px(24),Px(compact?372:352),Px(220),TRUE);
    MoveWindow(location,Px(24),Px(28),Px(compact?372:352),Px(26),TRUE);
    MoveWindow(stateText,Px(24),Px(76),Px(compact?292:276),Px(36),TRUE);
    MoveWindow(toggle,Px(compact?24:396),Px(compact?126:72),Px(compact?372:140),Px(42),TRUE);
    MoveWindow(mode,Px(compact?24:396),Px(compact?190:22),Px(compact?128:140),Px(32),TRUE);
    MoveWindow(pin,Px(170),Px(194),Px(146),Px(24),TRUE);
    MoveWindow(divider,Px(24),Px(140),Px(512),Px(1),TRUE);
    MoveWindow(netLabel,Px(24),Px(163),Px(512),Px(24),TRUE);
    MoveWindow(netEdit,Px(24),Px(196),Px(512),Px(76),TRUE);
    MoveWindow(help,Px(24),Px(282),Px(512),Px(42),TRUE);
    MoveWindow(save,Px(280),Px(330),Px(176),Px(34),TRUE);
    MoveWindow(autostart,Px(24),Px(footer),Px(512),Px(24),TRUE);
    MoveWindow(technical,Px(24),Px(footer+38),Px(248),Px(32),TRUE);
    MoveWindow(passwordButton,Px(288),Px(footer+38),Px(248),Px(32),TRUE);
    int passwordY=footer+82+(showDetails?112:0);
    MoveWindow(passwordLabel,Px(24),Px(passwordY),Px(512),Px(56),TRUE);
    MoveWindow(passwordEdit,Px(24),Px(passwordY+62),Px(512),Px(30),TRUE);
    MoveWindow(passwordCancel,Px(24),Px(passwordY+106),Px(248),Px(34),TRUE);
    MoveWindow(passwordSave,Px(288),Px(passwordY+106),Px(248),Px(34),TRUE);
    MoveWindow(details,Px(24),Px(footer+82),Px(512),Px(100),TRUE);
    MoveWindow(message,Px(24),Px(compact?224:footer+78+(showDetails?112:0)+(passwordOpen?174:0)),Px(width-48),Px(compact?18:48),TRUE);
    SendMessageW(select,CB_SETDROPPEDWIDTH,Px(650),0);
...
static void Refresh(bool fields=false) {
    ...
    try{
        ...
        if(fields||cached!=i||connected!=lastConnected) {
            ...
            SetWindowTextW(location,i>=0?ProfileLabel(profiles[i]).c_str():L"");
        ...
        EnableWindow(toggle,i>=0&&!working&&!dirty&&!passwordOpen);EnableWindow(select,!working&&!dirty&&!passwordOpen);
        EnableWindow(passwordButton,i>=0&&!active&&!working&&!dirty);EnableWindow(passwordSave,i>=0&&!active&&!working);
        EnableWindow(netEdit,i>=0&&!active&&!working&&!passwordOpen);EnableWindow(save,i>=0&&!active&&!working&&dirty);
...
static void Start(bool saving) {
    int i=Selection();if(i<0||working||passwordOpen)return;
...
static void ActivateWindow(bool widget) {if(widget)ClearPassword();compact=widget;Layout();ShowWindow(wnd,SW_RESTORE);SetForegroundWindow(wnd);Refresh();}
static void ReloadProfiles() {
    if(working||dirty||passwordOpen)return;auto old=Selection();std::wstring name=old>=0?profiles[old].name:L"";
    profiles=LocalProfiles();SendMessageW(select,CB_RESETCONTENT,0,0);int chosen=0;
    for(size_t i=0;i<profiles.size();i++){auto label=ProfileLabel(profiles[i]);SendMessageW(select,CB_ADDSTRING,0,reinterpret_cast<LPARAM>(label.c_str()));if(profiles[i].name==name)chosen=int(i);}
...
static LRESULT CALLBACK Proc(HWND h,UINT msg,WPARAM w,LPARAM l) {
    ...
    case WM_CREATE:{wnd=h;scale=GetDpiForWindow(h);background=CreateSolidBrush(RGB(255,255,255));
        ...
        passwordButton=control(L"BUTTON",L(L"Update password",L"Обновить пароль",L"Atjaunināt paroli"),WS_TABSTOP|BS_OWNERDRAW,248,372,208,32,211);
        passwordLabel=control(L"STATIC",L(L"Paste the password from your device card at vpn.tolf.is. This saves it in Windows; it does not change the server password.",L"Вставьте пароль из карточки устройства на vpn.tolf.is. Он будет сохранён в Windows; пароль на сервере здесь не меняется.",L"Ielīmējiet paroli no ierīces kartītes vietnē vpn.tolf.is. Tā tiks saglabāta Windows; servera parole šeit netiek mainīta."),0,24,414,432,56,0);
        passwordEdit=control(L"EDIT",L"",WS_BORDER|WS_TABSTOP|ES_PASSWORD|ES_AUTOHSCROLL,24,478,432,30,212);SendMessageW(passwordEdit,EM_SETLIMITTEXT,PWLEN,0);
        passwordSave=control(L"BUTTON",L(L"Save password",L"Сохранить пароль",L"Saglabāt paroli"),WS_TABSTOP|BS_OWNERDRAW,248,522,208,34,213);
        passwordCancel=control(L"BUTTON",L(L"Cancel",L"Отмена",L"Atcelt"),WS_TABSTOP|BS_OWNERDRAW,24,522,208,34,214);
    ...
    case WM_COMMAND:
        ...
        if(HIWORD(w)==BN_CLICKED){switch(LOWORD(w)){
            case 211:if(Selection()>=0&&!lastActive&&!working&&!dirty){passwordOpen=true;SetWindowTextW(message,L"");Layout();Refresh();SetFocus(passwordEdit);}break;
            case 214:ClearPassword();Layout();Refresh();SetFocus(passwordButton);break;
            case 213:{int i=Selection();if(i<0||working||lastActive)break;try{Secret password;password.value=Text(passwordEdit);UpdatePassword(profiles[i],password);ClearPassword();Layout();Refresh();SetWindowTextW(message,L(L"Password saved. You can connect now.",L"Пароль сохранён. Можно подключаться.",L"Parole saglabāta. Varat savienoties."));SetFocus(toggle);}catch(const Failure& f){ErrorText(f);}break;}
            case 208:showDetails=!showDetails;Layout();break;
            case 203:Start(true);break;
            case 204:Start(false);break;
            case 205:ClearPassword();compact=!compact;Layout();Refresh();break;
    ...
    case WM_DRAWITEM:{
        ...
        bool primary=d->hwndItem==toggle||d->hwndItem==save||d->hwndItem==passwordSave;
        bool danger=d->hwndItem==passwordCancel;
        COLORREF fill=disabled?RGB(246,247,248):primary?(pressed?RGB(58,64,70):RGB(35,40,45)):(pressed?RGB(242,244,246):RGB(255,255,255));
        COLORREF border=primary?fill:danger?RGB(194,34,42):RGB(216,220,224);
        ...
        SetBkMode(d->hDC,TRANSPARENT);SetTextColor(d->hDC,disabled?RGB(145,150,155):primary?RGB(255,255,255):danger?RGB(194,34,42):RGB(48,54,60));
    ...
    case WM_DESTROY:{ClearPassword();KillTimer(h,1);NOTIFYICONDATAW data={};data.cbSize=sizeof(data);data.hWnd=h;data.uID=1;Shell_NotifyIconW(NIM_DELETE,&data);DeleteObject(normal);DeleteObject(heading);DeleteObject(background);PostQuitMessage(0);return 0;}
    }return DefWindowProcW(h,msg,w,l);
}
inline int Run(HINSTANCE instance,bool widget,bool hidden) {
    HANDLE mutex=CreateMutexW(nullptr,FALSE,L"Local\\TOLF-VPN-Control-2.6.0");if(!mutex)return 1;
