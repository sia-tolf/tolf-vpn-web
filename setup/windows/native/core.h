...
inline void Fetch(const std::wstring& token,Settings& c){
 Http session(WinHttpOpen(L"TOLF-Setup/2.6.0",WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,WINHTTP_NO_PROXY_NAME,WINHTTP_NO_PROXY_BYPASS,0));
...
public:
 ...
 void route(const std::wstring& name,const std::wstring& prefix,bool add) {
  ...
  try{call(L"PS_VpnConnectionRoute",method,in.Get());}catch(const Failure&f){throw Failure{std::wstring(L"ROUTE_")+method+L" "+prefix,f.code};}
 }
