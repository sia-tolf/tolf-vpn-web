...
static void Start(bool load,bool connect=false){
 ...
 try{std::thread([load,connect,token,excluded]{auto r=std::make_unique<Result>();r->connect=connect;try{Com com;Wmi w;Preflight(w);if(load){r->loaded=std::make_unique<Settings>();Fetch(token,*r->loaded);r->networks=NetworkText(CheckedNetworks(SavedNetworks(r->loaded->id)));}else{r->name=L"TOLF - Riga - "+settings->id;ConfigureRoutes(w,*settings,r->name,excluded);r->networks=NetworkText(excluded);}}catch(const Failure&f){r->loaded.reset();r->error=Error(f);}catch(...){r->loaded.reset();r->error=Error({L"SETUP",ERROR_GEN_FAILURE});}PostMessageW(window,Done,0,reinterpret_cast<LPARAM>(r.release()));}).detach();}
...
static LRESULT CALLBACK Proc(HWND h,UINT m,WPARAM w,LPARAM l){switch(m){
 ...
 status=control(L"STATIC",L"",0,36,218,488,54,0,smallFont);footer=control(L"STATIC",L(L"Built into Windows  •  TOLF VPN 2.2",L"Средствами Windows  •  TOLF VPN 2.2",L"Windows līdzekļi  •  TOLF VPN 2.2"),SS_CENTER,36,276,488,22,0,smallFont);
