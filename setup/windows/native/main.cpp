...
static std::wstring Error(const Failure&f){
 ...
 else if(f.stage==L"CREDENTIALS")message=L(L"Saved credentials are unavailable. Load a new personal setup link.",L"Сохранённые учётные данные недоступны. Загрузите новую персональную ссылку настройки.",L"Saglabātie akreditācijas dati nav pieejami. Ielādējiet jaunu personīgo saiti.");
...
static void Start(bool load,bool connect=false){
 ...
 try{std::thread([load,connect,token,excluded]{auto r=std::make_unique<Result>();r->connect=connect;try{Com com;Wmi w;Preflight(w);if(load){r->loaded=std::make_unique<Settings>();Fetch(token,*r->loaded);r->networks=NetworkText(CheckedNetworks(SavedNetworks(r->loaded->id)));}else{r->name=L"TOLF - Riga - "+settings->id;ConfigureRoutes(w,*settings,r->name,excluded);InstallController();r->networks=NetworkText(excluded);if(connect)DialProfile(r->name);}}catch(const Failure&f){r->loaded.reset();r->error=Error(f);}catch(...){r->loaded.reset();r->error=Error({L"SETUP",ERROR_GEN_FAILURE});}PostMessageW(window,Done,0,reinterpret_cast<LPARAM>(r.release()));}).detach();}
