...
def supported_modes():
    advertised = routes.available(CTX)
    return {key: advertised.get(key, [value[1]]) for key,value in NODES.items()}
...
def package(row, credentials, lang):
    ...
    with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr('README.txt', ('\n\n'.join([TEXT[lang][0], TEXT[lang][2], route_description(node, row['local_id'], lang), TEXT[lang][3]])).encode('utf-8-sig'))
    profiles = CTX['tolf_profiles']
    profiles.initialize()
    token = secrets.token_urlsafe(24)
    content = output.getvalue()
    metadata = {'platform': 'windows', 'deviceId': row['id'], 'name': row['name'],
                'username': username, 'server': node, 'localId': row['local_id'], 'language': lang, 'expiresAt': (now()+timedelta(hours=24)).isoformat(),
...
def install(app, context):
    ...
    def capabilities():
        ...
        return {'version': VERSION, 'servers': list(NODES), 'routing': 'sr', 'routingModes': modes, 'routingRevision': ROUTING_REVISION, 'installerVersion': '2.6.1', 'profileLabels': True, 'passwordManagement': True}
    ...
    def create(request: Request, payload: dict):
        ...
        node, mode = routes.selection(payload)
        ...
        with CTX['tolf_promos'].account_operation(CTX['DB'], user_id):
            with db() as c:
                ...
                if not existing:
                    ...
                    c.execute('INSERT INTO windows_device_routes VALUES (?,?,?,?)', (device_id, node, mode, managed))
            row = owned(user_id, device_id)
            if row['server'] != node:
    ...
    def page(token: str):
        ...
        labels[6] = route_description(metadata.get('server', 'riga'), metadata.get('localId', NODES[metadata.get('server', 'riga')][1]), lang)
        copy = {
