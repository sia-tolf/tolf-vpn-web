...
TERMINAL = {'invalid_invitation','invitation_used','invitation_expired','already_linked',
            'account_has_vpn','user_not_found','unsupported_credential_file'}
...
def claim(request: Request, payload: dict):
    ...
    with CTX['tolf_promos'].account_operation(CTX['DB'],user_id):
        with connect() as con:
            ...
            if previous:
                if previous[0] != digest:
                    raise HTTPException(409, 'Finish the previous invitation first')
                if previous[1] == 'ready':
        ...
        with connect() as con:
            ...
            con.execute("UPDATE vpn_imports SET state='ready',username=?,protected=? WHERE user_id=?",
                        (value['username'],int(value['protected']),user_id))
...
def install(app, context):
    ...
    app.get('/vpn/invitations/capabilities')(lambda: {'version': 1})
