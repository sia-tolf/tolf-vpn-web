...
function getSelectedLocalId() {
  ...
  return (
    (localIdValues[server] || "")
      .trim()
      .toLowerCase() ||
    LOCAL_ID_DEFAULTS[server]
  );
}
...
function renderLocalIdSettings() {
  ...
  const values = Object.keys(options)
    .map(value => value || t("localIdEmpty"))
    .join(", ");
  ...
  localIdInput.placeholder = t(
    server === "riga"
      ? "localIdDefaultRiga"
      : "localIdDefaultMoscow"
  );

  localIdInput.disabled = vpnBusy;

  localIdChoices.textContent =
    t("localIdChoices", { values });
...
localIdInput?.addEventListener("input", () => {
  localIdValues[getSelectedServerKey()] =
    localIdInput.value;
